...
if __name__  == "__main__":
    ...
    try:
        ...
        #shutil.move(origSam, "/".join([args.alignmentParDir, swappedDataSet, "orig"])  # TODO: need to do this file by file to rename at the same time

        for subdir in next(os.walk(origSam))[1]:  # just get the top level subdirectories
            if subdir in ["genome", "junction", "reg", "ribo", "transcriptome", "denovo"]:  # if it is one of the alignment output directories
                for f in os.listdir("/".join([origSam, subdir])):
                    ...
                    print "/".join([origSam, subdir, f]), "is moving to", "/".join([args.alignmentParDir, swappedDataSet, "orig", subdir, newf])
                    shutil.move("/".join([origSam, subdir, f]), "/".join([args.alignmentParDir, swappedDataSet, "orig", subdir, newf]))
