...
if [ $# -eq 6 ]
then
  DENOVO_ID_DIR=$ID_DIR/$DENOVO_DIRNAME$6
  mkdir -p $DENOVO_ID_DIR
  
  JUNC_ID_DIR=$ID_DIR/$JUNC_DIRNAME$6
  mkdir -p $JUNC_ID_DIR
  
  REG_ID_DIR=$ID_DIR/$REG_DIRNAME$6
  mkdir -p $REG_ID_DIR
  
  mkdir -p $ID_DIR/$JUNC_NONGR_DIRNAME$6
else
...
then
  ...
  then
    DO_CONVERT=true
    echo "planning to convert denovo ${f}"
